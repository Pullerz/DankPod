...
class GenresViewController: UIViewController, ClickWheelProtocol {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        AppDelegate.baseVC.navBarTitle = "Genres"
        getGenres()
    }
    
    func getGenres() {
        menuItems = AppDelegate.music.getGenres()
    ...
    func centerPressed(pressure: ClickWheelTouchPressure) {
        if pressure == .NORMAL {
            ...
            let genres = AppDelegate.music.getSongsByGenre(genre: menuItems[counter])
            var songs: [MPMediaItem] = []
            for genre in genres {
                for song in genre.items {
                    songs.append(song)
                }
            }
            vc.menuItems = songs
